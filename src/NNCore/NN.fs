...
module NN =
    ...
    let predict thetasVector layers X = 
        let thetas = allocateThetas thetasVector layers    
        let (a,z) = calcLayers thetas X 
        let h = z |> List.head |> Matrix.map sigmoid
        ...
        seq { for i in 0..h.NumRows-1 do
                yield h.Row i |> RowVector.transpose |> Vector.foldi getMax (-1,-1.0) |> fst |> float } 

    let makeList X = 
        [X |> Array.toList]

    let buildPredictor thetasLoc layers =
        let thetas = load thetasLoc ' '
        fun X -> predict thetas (layers |> Array.toList) X |> Seq.head

    let accuracy thetasVector X (y:matrix) layers =
        predict thetasVector layers X |> Seq.zip y |> Seq.map (fun (f,s) -> if f=s then 1.0 else 0.0 ) |> Seq.average

    let splitToTrainAndValidationSet (X:matrix) Y xrowsToTrain=
